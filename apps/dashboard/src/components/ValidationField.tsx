...
export function ValidationIndicator({ status }: { status: ValidationStatus }) {
  return (
    <span className={`mode-badge ${(status === 'checking_local' || status === 'checking_ai') ? 'pause' : status === 'success' ? 'blur' : status === 'idle' ? 'submit' : 'pause'}`}>
...
function InlineToast({
  ...
  if (status === 'checking_local' || status === 'checking_ai') {
  ...
  return (
    ...
    >
      {status === 'checking_local' || status === 'checking_ai' ? (
