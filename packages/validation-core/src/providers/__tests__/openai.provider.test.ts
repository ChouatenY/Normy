...
describe('OpenAIProvider', () => {
  ...
  it('throws INVALID_MODEL_CONFIGURATION on model not found error (404)', async () => {
    ...
    const provider = new OpenAIProvider({ provider: 'openai', apiKey: 'test-key', model: 'invalid-model', maxRetries: 0 });
  ...
  it('gracefully falls back to secondary model if first model is not found', async () => {
    ...
    const provider = new OpenAIProvider({ provider: 'openai', apiKey: 'test-key', maxRetries: 0 });
    const result = await provider.validate({
      question: 'Q',
      answer: 'A',
    });
